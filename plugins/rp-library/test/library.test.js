...
test('用户界面错误文案不暴露服务端错误码和内部消息', () => {
  ...
  assert.equal(userErrorMessage({ code: 'RP_CONTEXT_SOURCE_REQUIRED' }, 'save'), '对话历史和当前输入必须参与回复，不能放入闲置区。')
...
test('会话设置与会话 Wiki 不承担资料创建、导入或编辑', async () => {
  ...
  assert.match(client, /这里仅供查看，资料内容请在侧栏资料库中管理/)
...
test('资料导航、会话 Wiki 与 Prompt 入口清晰分工且不泄漏内部错误', async () => {
  ...
  assert.match(client, /function RpMobileWorkbenchControl[\s\S]*?className: css\.workbenchMobileControls[\s\S]*?h\(SessionWikiTrigger/)
  ...
  assert.match(contextCanvas, /拖动左侧手柄排序，拖动分组名称可移入闲置区；拖动资料可更换分组。对话历史和当前输入始终启用。/)
  ...
  assert.match(styles, /\.workbenchMobileControls \{ display: grid; grid-template-columns: repeat\(2, 44px\); width: calc\(100% - var\(--dsh-composer-side-clearance\) - var\(--dsh-composer-side-clearance\)\); max-width: var\(--dsh-composer-card-max-width\); align-items: center; justify-content: end;/)
