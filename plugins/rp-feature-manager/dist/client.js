...
get: (_target, key) => {
		...
		*/
		...
		async function setReplyOptionsSettings(connection, count, maxCharacters, keywords, expectedRevision) {
		...
		const css = {
			...
			"quickCursorField": "rp-features-quickCursorField",
			...
			"replyOptionsBasicField": "rp-features-replyOptionsBasicField",
			"replyOptionsBasicFields": "rp-features-replyOptionsBasicFields",
			"replyOptionsBasicSection": "rp-features-replyOptionsBasicSection",
			...
			"replyOptionsNumberInput": "rp-features-replyOptionsNumberInput",
			"replyOptionsOverlayState": "rp-features-replyOptionsOverlayState",
		...
		*/
		...
		function ReplyOptionsSettingsDialog({ open, count, maxCharacters, keywords, writable, saving, onSave, onClose }) {
			...
			const currentMaxCharacters = validMaxCharacters(maxCharacters) ? maxCharacters : 50;
			...
			const [maxCharactersDraft, setMaxCharactersDraft] = (0, react.useState)(String(currentMaxCharacters));
			...
			const validationError = parsedCount === void 0 ? "请输入 1 到 5 之间的整数。" : parsedMaxCharacters === void 0 ? `请输入 1 到 200 之间的整数。` : keywordErrorIndex >= 0 ? `选项 ${keywordErrorIndex + 1} 的方向关键词最多 40 个字符。` : "";
			...
			const changed = parsedCount !== void 0 && parsedMaxCharacters !== void 0 && nextKeywords !== void 0 && (parsedCount !== currentCount || parsedMaxCharacters !== currentMaxCharacters || !sameStrings(nextKeywords, currentKeywords));
			if (!open) return null;
			const save = async () => {
				if (!writable || saving || parsedCount === void 0 || parsedMaxCharacters === void 0 || nextKeywords === void 0 || !changed) return;
				setSaveError("");
				try {
					if (await onSave({
						count: parsedCount,
			...
			return h$1(_deepseek_ai_dsh_client_ui_primitives.Modal, {
			...
			}, h$1("div", { className: css.replyOptionsBasicSection }, h$1("div", { className: css.replyOptionsBasicFields }, h$1("label", { className: css.replyOptionsBasicField }, h$1("span", null, "回复条数"), h$1("span", { className: css.replyOptionsNumberInput }, h$1("input", {
				type: "number",
				inputMode: "numeric",
				min: 1,
				max: 5,
				...
				"aria-describedby": `${basicsHelpId} ${errorId}`,
				onInput: (event) => {
					setMaxCharactersDraft(event.currentTarget.value);
					setSaveError("");
				}
		...
		function parseMaxCharactersDraft(value) {
			if (typeof value !== "string" || !/^\d{1,3}$/u.test(value.trim())) return void 0;
			const parsed = Number(value.trim());
			return validMaxCharacters(parsed) ? parsed : void 0;
		}
		...
		function ReplyOptionsSettingsOverlay({ controller, connection }) {
			const command = (0, react.useSyncExternalStore)(controller.subscribe, controller.getSnapshot);
			const [status, setStatus] = (0, react.useState)({ phase: "idle" });
			const [saving, setSaving] = (0, react.useState)(false);
			(0, react.useEffect)(() => {
				if (!command.open) return void 0;
				let live = true;
				setSaving(false);
			...
			const writable = status.value.compatible === true && status.value.settings?.writable === true && Number.isSafeInteger(settingsRevision);
			const save = async ({ count, maxCharacters, keywords }) => {
				if (!writable || saving) return false;
				setSaving(true);
				try {
					const nextStatus = await setReplyOptionsSettings(connection, count, maxCharacters, keywords, settingsRevision);
					if (nextStatus.replyOptionsCount !== count || nextStatus.replyOptionsMaxCharacters !== maxCharacters || !sameSelection(nextStatus.replyOptionsKeywords, keywords)) throw new Error("Roleplay reply option settings were not applied");
					setStatus({
						phase: "ready",
						value: nextStatus
					});
		...
		//#endregion
		exports.ReplyOptionsSettingsController = ReplyOptionsSettingsController;
		exports.ReplyOptionsSettingsOverlay = ReplyOptionsSettingsOverlay;
