...
test('roleplay request envelope gives typed inputs, generic specialist routing and protected context boundaries', () => {
  ...
  assert.match(text, /<section name="人设信息"><item name="我的人设">/)
  assert.match(text, /If that persona item is absent, infer the user-controlled protagonist/)
  ...
  assert.match(chatText, /<section name="人设信息"><item name="我的人设">/)
  assert.match(chatText, /that item's content defines the user-controlled protagonist/)
  assert.match(chatText, /If that persona item is absent, infer the user-controlled protagonist/)
  assert.match(chatText, /Do not choose the user-controlled protagonist from character-card or scenario material/)
...
test('settings preview is projected from the same runtime prompt functions', () => {
  ...
  const chatToolSchema = chat.layers.find(layer => layer.id === 'tool-schema')
  const agentToolSchema = agent.layers.find(layer => layer.id === 'tool-schema')
  assert.equal(agentToolSchema.contentKind, 'derived')
  assert.equal(chatToolSchema.tools.includes('bash'), false)
  assert.equal(chatToolSchema.tools.includes('pwsh'), false)
  assert.equal(chatToolSchema.tools.includes('str_replace_editor'), false)
  assert.ok(agentToolSchema.tools.includes(process.platform === 'win32' ? 'pwsh' : 'bash'))
  assert.ok(agentToolSchema.tools.includes('str_replace_editor'))
})
