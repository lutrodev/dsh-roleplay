...
export const ROLEPLAY_PROMPT_PREVIEW_VERSION = 12
...
export function roleplayPersonaText({ stateEnabled = false } = {}) {
  return [
    ...
    stateEnabled
      ? 'For ordinary narrative, use the supplied state_commit_contract directly and do not call rp_state_read. Use rp_state_read only when that contract is absent or the user explicitly asks to inspect State. Configure State definitions only in Agent mode after an explicit user request and after loading rp-guide-state; ordinary in-story changes belong in state.update effects when narrative work is committed.'
      : undefined,
