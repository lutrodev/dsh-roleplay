...
test('imports an MVU+lore card, creates an Actor session and commits atomically', async () => {
  ...
  try {
    ...
    assert.ok(stateFragment?.text.includes('"description": "current health"') && stateFragment.text.includes('"hp": 6'))
    assert.match(stateFragment.text, /"expectedRevision": 2/)
    assert.match(stateFragment.text, /"kind": "state\.update"/)
    assert.doesNotMatch(stateFragment.text, /initialValue|ValueWithDescription|state\.patch|payload\.namespace|writable/)
