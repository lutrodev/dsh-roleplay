...
test('会话变量条件明确展示分组与完整路径，并将组合表达式收进高级设置', async () => {
  const client = await readFile(new URL('../src/client.js', import.meta.url), 'utf8')
  const styles = await readFile(new URL('../src/client.module.css', import.meta.url), 'utf8')
  assert.match(client, /function StateConditionEditor/)
  assert.match(client, /h\('strong', null, '会话变量条件'\)/)
  assert.match(client, /这里只设置使用条件，不会创建变量/)
  assert.match(client, /'aria-label': '变量分组'/)
  assert.match(client, /'aria-label': '完整变量路径'/)
  assert.match(client, /'aria-label': '变量比较方式'/)
  assert.match(client, /'aria-label': '变量比较值'/)
  assert.match(client, /格式示例：story · \/plot\/progress/)
  assert.match(client, /\/main\/progress、\/side\/progress/)
  assert.match(client, /state\("story", "\/plot\/progress"\) >= 50/)
  assert.doesNotMatch(client, /李钰/)
  assert.match(client, /'高级条件'/)
  assert.match(client, /'返回常用设置'/)
  assert.match(styles, /\.conditionAddressFields,.conditionCompareFields\s*\{[^}]*display:grid;/)
