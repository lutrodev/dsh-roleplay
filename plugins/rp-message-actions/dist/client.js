...
get: (_target, key) => {
		...
		*/
		...
		function assistantMessageContent(messageRow) {
			const content = (messageRow?.querySelector?.("[data-slot=\"conversation.chat.node\"]"))?.firstElementChild;
		...
		function userMessageContentStack(messageRow) {
			const stack = (messageRow?.querySelector?.("[data-actions-reveal]"))?.firstElementChild;
