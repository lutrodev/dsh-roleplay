...
test('creates a truly blank preset or copies the example preset', async () => {
  ...
  try {
    ...
    assert.match(writingGuidance, /结尾应停在场景仍有行动惯性、用户可以自然接手的位置/)
    assert.match(writingGuidance, /不要把场景写尽/)
    assert.match(writingGuidance, /避免用升华、总结或解释意义来封口/)
    assert.match(writingGuidance, /对照当前可见对话历史中近期回复的结尾/)
    assert.match(writingGuidance, /避免重复无法产生新作用的形式或内容/)
    assert.match(writingGuidance, /句式骨架、段落节奏、人物动作、对白方式、环境意象、情绪落点、悬念设置或叙事功能/)
    assert.match(writingGuidance, /延续、伏笔回收或有意呼应不算重复/)
    assert.match(writingGuidance, /最后一段是否提供了继续所需的信息，并留下不止一种自然接续方式/)
