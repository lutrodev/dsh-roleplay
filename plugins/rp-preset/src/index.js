...
结尾应停在场景仍有行动惯性、用户可以自然接手的位置。让本轮产生的反应、后果、发现、关系变化或压力形成具体的接续点；接续下一步所必需的时空、在场人物和局势变化应当足够明确，但不要为此复述整个场景。

不要把场景写尽。不要一并处理所有人的反应、善后和余波，也不要在用户能够介入之前擅自跳到稍后或下一幕。给用户留下真实可行动的空间，而不是只剩接受既定结果。

避免用升华、总结或解释意义来封口。不要在末段概括人物成长、关系本质、事件教训或时代命运，也不要用“这一刻他终于明白”“一切从此不同”“故事才刚刚开始”之类结论替代正在发生的故事。情绪和主题应留在人物行为与具体后果中。

对照当前可见对话历史中近期回复的结尾，避免重复无法产生新作用的形式或内容。仅替换少量词语，却沿用高度相似的句式骨架、段落节奏、人物动作、对白方式、环境意象、情绪落点、悬念设置或叙事功能，仍然属于重复。由当前因果自然形成的延续、伏笔回收或有意呼应不算重复；不要为了表面不同而牺牲连续性或强造变化。

最后一句应属于当前场景，而不是作者评语。它可以落在其他角色尚未完成的反应、局势中可感知的改变、迫近的具体压力或一个仍在发展的动作上，但不要把任何一种落点写成固定公式。完成后检查：最后一段是否提供了继续所需的信息，并留下不止一种自然接续方式，而不是重新罗列状态或把用户推向唯一结果。
...
export async function apply(ctx, config) {
  ...
  await presets.ensureDefault()
  if (config.exposeBrowser !== false) ctx.inject(['connection'], browserCtx => registerBrowser(browserCtx, presets))
}

function registerBrowser(ctx, presets) {
