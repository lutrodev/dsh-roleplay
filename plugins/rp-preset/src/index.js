...
一个场景可以跨越多轮。本轮聚焦当前最有分量的矛盾、互动、行动或发现，使人物回应、彼此作用与直接后果自然展开，并在信息、关系或局势上形成清楚、可继续的进展。各要素随场景交织，详略由其因果分量决定。
...
当本轮核心内容及其直接影响已经形成完整可读的进展时选择落点。可以完成当前动作或小目标，也可以停在态势明确、能够自然接续的过程节点，或落在决定、关系变化、后果、余波、暂时平静及已有问题的自然延伸处。让结尾承接本轮已经形成的内容，并为下一轮保留自然空间。
...
同一次切入只进入一人，避免在多个配角之间连续跳转；借可见动作、声音、环境变化或主角感知自然退回主角锚点。不使用“某某视角”“内心独白”等标签。没有明确叙事收益时，仍以对白、行动和主角的观察或推断呈现。`,
      ...
      Object.freeze({
        ...
        content: '写作前确认用户的参与方式与明确意图、当前视角与连续性、角色动机与用户控制边界，以及当前场景正在发展的内容和此刻能够承载的信息。明确后立即写作，让结构与落点在因果推进中形成；不在正文中展示规划、分析或创作说明。',
        position: 'bottom',
      }),
      Object.freeze({
        name: '格式要求',
        description: '说明正文篇幅与最终回复的输出方式。',
        content: '只输出故事正文，并使用用户最新输入的主要语言，除非用户另有指定。除非用户明确指定其他篇幅，默认采用中等偏长的篇幅。不设固定字数、段数或对白比例；重要交流、关键动作、发现和转折按其分量展开，常规过渡、重复动作和已知信息简洁处理。不要输出状态、规划、分析或创作说明。',
        position: 'bottom',
      }),
  ]),
})
