...
同一次切入只进入一人，避免在多个配角之间连续跳转；借可见动作、声音、环境变化或主角感知自然退回主角锚点。不使用“某某视角”“内心独白”等标签。没有明确叙事收益时，仍以对白、行动和主角的观察或推断呈现。`,
      ...
      Object.freeze({
        ...
        content: '写作前确认用户的参与方式与明确意图、当前视角与连续性、角色动机与用户控制边界，以及当前场景的主要推进、此刻能够承载的信息和自然落点。明确后立即写作，让结构与落点在因果推进中形成；不在正文中展示规划、分析或创作说明。',
        position: 'bottom',
      }),
      Object.freeze({
        name: '格式要求',
        description: '说明正文篇幅与最终回复的输出方式。',
        content: '只输出故事正文，并使用用户最新输入的主要语言，除非用户另有指定。除非用户明确指定其他篇幅，默认采用与当前内容相称的中等篇幅。不设固定字数、段数或对白比例；重要互动、关键动作及其直接影响按其分量展开，常规过渡、重复动作和已知信息简洁处理。不要输出状态、规划、分析或创作说明。',
        position: 'bottom',
      }),
  ]),
})
