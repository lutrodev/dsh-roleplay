...
| `rp-reply-options` | 在成功提交的主 Roleplay 剧情回复中要求主模型同时给出设置的 1–5 条第三人称主角消息（默认 3），以主角姓名或代词为叙述主体，只写主角接下来的对白和／或行为；第一人称仅可出现在引号内对白中，同时禁止导演式续写、替其他角色反应或预定行动结果；每条 Unicode 字符上限指导可设为 1–200（默认 50），该值只写入 Prompt，提交与回放不按选项文本长度拒绝内容；配置条数作为生成目标和规范结果上限，至少一条可用选项即可提交，空白、重复和超额项由 validator 规范化；扩展 Schema 显式接收额外注释字段但只持久化 `options`。`user-controlled protagonist` 由上下文说明静态指向 `<section name="人设信息"><item name="我的人设">`，不动态拼接具体人设；标签缺失时模型从其余上下文与对话推断。每个编号还可配置一个不超过 40 字符的可选方向关键词，三项设置原子保存并经 `rp-standard` 写入 preset 运行时，关键词只约束对应选项且不进入输出；浏览器从事件日志投影最新可操作节点并添加不进入消息正文的数字编号，卡片齿轮通过 Host 全局设置服务打开同一设置浮层，点击选项则原样调用当前 Session 的 `conversation.send()` | Settings `replyOptionsCount` + `replyOptionsMaxCharacters` + `replyOptionsKeywords`、`rpReplyOptionsSettings`、必需 artifact extension `rp.reply-options`、`rp_commit_turn.extensions`、`conversation.chat.node`、Session-scoped Conversation |
...
5. Writer 工具结果尚未展示给用户。Chat 的父 Agent 在下一次模型输出中只生成 `rp_commit_turn` 参数，运行时把 Writer 原文作为唯一正文流式注入并抑制父模型正文；Agent 的父 Agent 在初稿后完成全部适用的后置契约，审阅返回材料并确定最终正文。前后顺序来自各目录项的自然语言契约，不是 Core 自动工作流。Chat 由运行时把正文与纯提交输出合并为同一条原生助手消息，Agent 在完整正文末尾放置唯一 `rp_commit_turn`。commit 必须关联本轮唯一成功 Writer artifact；Chat 采用运行时注入的 Writer 正文，Agent 采用父 Agent 的最终可见正文。每个 effect 能力注册一个根对象闭合、`kind` 固定的 schema，并可选注册只读 `diagnoseArguments` 纠错器；每个 artifact extension 默认注册根对象闭合的 schema，并可声明为必需，只有拥有明确规范化 validator 的扩展才可显式接受 namespace 内额外属性。Core 按当前注册表动态组装提交工具的 `effects.items` 与顶层闭合的 `extensions` 命名空间，在注册／卸载时发送 `tools/change`，未知命名空间、必需扩展缺失或无法规范化的字段仍阻断提交。State 的 `set/increment/append/remove` 因此在模型生成参数前就拥有不同的必填字段和允许字段；启用回复选项时，主模型也在同一次提交中生成必需的 `rp.reply-options`，已填写的编号方向关键词由 preset 固化进该扩展的动态 Schema，不增加新的模型调用或旁路事实源。参数或领域校验失败以稳定 JSON 返回错误类别、代码和字段级纠错信息；Chat 的工具专用重试会从原生失败结果关联此前正文，不再次输出正文。成功 `tool/result.meta.kind=rp-agent/turn-commit` 再原子承载 effect、reference、extension、上下文与 Writer 引用。
