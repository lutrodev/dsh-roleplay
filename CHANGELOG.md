...
- 微调内置“示例预设”的篇幅与收束指导：保留现有长程推进结构，不再默认要求“中等偏长”，并将单轮范围明确为一个主要推进点及其直接影响，避免连续跨越多个叙事阶段；抽象的收束说明改为具体写作动作，要求写清变化与直接影响、按既有事实和因果组织、不机械套用近期结构，也不另加旁白总结或点明寓意；未编辑的托管旧示例会保留栏位 ID 与分组标签设置并升级内容。
- 迁移到 DSH `0.1.2-alpha.5`：Session Event Log 使用 `snapshotEvents()`、`eventAt()`、`seq`、`isSeeded` 与精确的 `inheritedEventCount`；角色删除预览的冷会话扫描改用 `list()` 快照和显式关闭的只读 Session handle，真实 JSONL 回归测试同步覆盖新的持久化所有权边界；Session 与结构化子代理调用继续集中到 Core 薄适配层，全部 DSH 开发依赖和 peer dependency 精确锁定同一版本。现有 JSONL 会话格式保持兼容，无需数据改写。
